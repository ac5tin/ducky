...
export function ConnectorsView() {
  ...
  const connect = async (s: ServerSummary) => {
    try {
      ...
      if (result === "needs_auth") {
        ...
        toast("error", `${s.name} failed to connect — see its detail view for logs.`);
      }
    } catch (e) {
      toast("error", `${e}`);
    }
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-4xl px-6 py-8">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm leading-relaxed text-slate-500 dark:text-slate-400">
              Connectors are MCP servers that give your AI tools, data and prompts.
              Everything runs with your approval.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-3">
          {servers.length === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-slate-200 p-10 text-center dark:border-slate-800">
              ...
              <p className="mx-auto mt-1 max-w-sm text-xs leading-relaxed text-slate-400">
                Try “Filesystem” to let the AI read files, or “Fetch” to read web pages.
                You can also paste a config from another MCP app.
              </p>
          ...
          {servers.map((s) => {
            ...
            const needsAuth = typeof s.status === "object" && "needs_auth" in s.status;
            return (
              <div
                key={s.id}
              ...
              >
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300">
                  <Icon name={s.transport_kind === "http" ? "globe" : "terminal"} className="h-5 w-5" />
                </span>
                <button className="min-w-0 flex-1 text-left" onClick={() => setDetailId(s.id)}>
                  ...
                  <div className="mt-0.5 truncate text-xs text-slate-400">
                    {st.text}
                    {typeof s.status === "object" && "error" in s.status && ` — ${s.status.error.message}`}
                    {" · "}
                    {s.tools.length} tools{s.prompts.length > 0 ? ` · ${s.prompts.length} prompts` : ""}
                  </div>
                </button>
                <div className="flex shrink-0 items-center gap-1.5">
                  ...
                  {!needsAuth && s.enabled && (
                    <Button
                      variant="secondary"
                      onClick={() =>
                        s.status === "connected" ? api.mcpDisconnect(s.id).then(refreshServers) : connect(s)
                      }
                    >
                      {s.status === "connected" ? "Disconnect" : "Connect"}
                    </Button>
                  )}
                  <label className="ml-1 cursor-pointer" title={s.enabled ? "Disable" : "Enable"}>
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="relative block h-5.5 w-10 rounded-full bg-slate-300 transition peer-checked:bg-sky-500 dark:bg-slate-700 after:absolute after:left-0.5 after:top-0.5 after:h-4.5 after:w-4.5 after:rounded-full after:bg-white after:transition after:peer-checked:translate-x-4" />
                  </label>
                </div>
              </div>
            );
      ...
      <ServerDetailModal serverId={detailId} onClose={() => setDetailId(null)} />
    </div>
  );
}
