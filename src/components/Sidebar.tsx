...
export function Sidebar() {
  ...
  const titleGeneratingIds = useStore((s) => s.titleGeneratingIds);
        ...
        {conversations.map((c) => (
          ...
          >
              ...
              <>
                ...
                <button
                  ...
                  onMouseDown={(e) => {
                    ...
                    skipBlur.current = true;
                    setEditingId(null);
                    generateTitle(c.id).catch((err) => console.error(err));
                  }}
                >
                  <Icon name="refresh" className="h-3.5 w-3.5" />
                </button>
              </>
            ) : titleGeneratingIds.has(c.id) ? (
              <button
                className="min-w-0 flex-1 cursor-pointer truncate text-left"
                title="Click to name this chat"
                onClick={() => {
                  openConversation(c.id).catch((e) => console.error(e));
                  cancelTitle(c.id).catch((e) => console.error(e));
                  setEditingId(c.id);
                  setDraft(c.title);
                }}
              >
            ...
            <button
              ...
              onClick={() => {
                if (titleGeneratingIds.has(c.id)) {
                  cancelTitle(c.id).catch((e) => console.error(e));
                }
                setEditingId(c.id);
                setDraft(c.title);
